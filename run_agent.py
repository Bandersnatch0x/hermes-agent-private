...
class AIAgent:
    ...
    def __init__(
        ...
        if self.api_mode == "anthropic_messages":
            ...
            if api_key and base_url:
                ...
                elif base_url_host_matches(effective_base, "windhub.cc"):
                    client_kwargs["default_headers"] = {
                        "User-Agent": "claude-code/0.1.0",
                    }
    ...
    def _create_openai_client(self, client_kwargs: dict, *, reason: str, shared: bool) -> Any:
        ...
        if "http_client" not in client_kwargs:
            ...
            try:
                from providers import get_provider_profile as _gpf3, list_providers as _lpp3
                _base_url = client_kwargs.get("base_url", "") or ""
                _ph3 = _gpf3(self.provider)
                # If the configured provider has no factory, look for any plugin
                # that claims this base_url's hostname — lets users wire a
                # transport plugin (e.g. windhub) without renaming the provider.
                if not (_ph3 and _ph3.http_transport_factory) and _base_url:
                    from utils import base_url_host_matches as _bhm3
                    for _pp3 in _lpp3():
                        if not _pp3.http_transport_factory:
                            continue
                        _host = _pp3.get_hostname()
                        if _host and _bhm3(_base_url, _host):
                            _ph3 = _pp3
                            break
                if _ph3 and _ph3.http_transport_factory:
                    _proxy = _get_proxy_for_base_url(_base_url)
                    _factory_client = _ph3.http_transport_factory(_base_url, _proxy)
            except Exception:
                _factory_client = None
            if _factory_client is not None:
                client_kwargs["http_client"] = _factory_client
            else:
