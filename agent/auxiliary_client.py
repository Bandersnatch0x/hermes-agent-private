...
class _OpenAIProxy:
    ...
    def __call__(self, *args, **kwargs):
        # If the caller didn't supply http_client, look up a provider profile
        # whose hostname matches kwargs['base_url'] and has a
        # http_transport_factory declared (plugins like windhub use this to
        # strip x-stainless-* / rewrite User-Agent, both fingerprints that
        # some gateways block). Without this injection, all 15+ auxiliary
        # OpenAI(...) construction sites bypass the factory and get blocked
        # by the gateway even though main-agent calls succeed.
        if "http_client" not in kwargs:
            base_url = kwargs.get("base_url") or ""
            if base_url:
                try:
                    from providers import list_providers as _lp
                    from utils import base_url_host_matches as _bhm
                    for _pp in _lp():
                        if not _pp.http_transport_factory:
                            continue
                        _host = _pp.get_hostname()
                        if _host and _bhm(base_url, _host):
                            try:
                                _hc = _pp.http_transport_factory(base_url, None)
                            except Exception:
                                _hc = None
                            if _hc is not None:
                                kwargs["http_client"] = _hc
                            break
                except Exception:
                    pass
